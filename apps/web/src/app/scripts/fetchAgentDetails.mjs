...
import fetch from 'node-fetch';
import { PrismaClient } from '@prisma/client';
import { getPromptBase, getPromptBaseV2, parseAgentDetails } from './constants.mjs';
...
async function fetchAndStoreAgentDetails() {
  try {
    ...
    for (const tool of tools) {
      ...
      console.log(`Fetching details for: ${agentName}`);
      const promptBase = getPromptBaseV2(agentName)
      ...
      await prisma.tool.update({
        ...
        data: {
          ...
          firstCommitDate: fetchedData.firstCommitDate ? new Date(fetchedData.firstCommitDate) : null,
          lastCommitDate: fetchedData.lastCommitDate ? new Date(fetchedData.lastCommitDate) : null,
